...
void cl_init_create_buffers(){
 ...
 // Create buffers
  ...
  x_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, (num_nodes_host+1) * sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (x_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }

  y_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, (num_nodes_host+1) * sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (y_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }

  z_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, (num_nodes_host+1) * sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (z_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }

  vx_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, (num_bodies_host) * sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (vx_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }

  vy_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE , (num_bodies_host) * sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (vy_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }

  vz_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, (num_bodies_host) * sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
  ...
  rad_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, (num_bodies_host) * sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
  ...
  softening2_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr, "clCreateBuffer Error (softening2_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }

  inv_opening_angle2_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS){
    fprintf(stderr, "clCreateBuffer Error (inv_opening_angle2_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }

  minimum_collision_velocity_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS){
  ...
  num_nodes_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_int), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (num_nodes_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  num_bodies_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_int), NULL,  &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (num_bodies_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  boxsize_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (boxsize_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  rootx_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (rootx_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  rooty_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (rooty_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  rootz_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (rootz_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  OMEGA_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (OMEGA_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  OMEGAZ_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (OMEGAZ_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  sindt_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (sindt_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  tandt_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (tandt_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  sindtz_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (sindtz_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  tandtz_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (tandtz_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  t_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (t_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  collisions_max2_r_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (collisions_max2_r_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }

  dt_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (dt_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }

  G_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float), NULL, &error);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clCreateBuffer ERROR (dt_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
...
void cl_init_write_buffers(){

  error = clEnqueueWriteBuffer(queue, x_buffer, CL_TRUE, 0, (num_nodes_host+1) * sizeof(cl_float), x_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (x_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, y_buffer, CL_TRUE, 0, (num_nodes_host+1) * sizeof(cl_float), y_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (y_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, z_buffer, CL_TRUE, 0, (num_nodes_host+1) * sizeof(cl_float), z_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (z_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, vx_buffer, CL_TRUE, 0, (num_bodies_host) * sizeof(cl_float), vx_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (vx_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, vy_buffer, CL_TRUE, 0, (num_bodies_host) * sizeof(cl_float), vy_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (vy_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, vz_buffer, CL_TRUE, 0, (num_bodies_host) * sizeof(cl_float), vz_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (vz_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, ax_buffer, CL_TRUE, 0, (num_bodies_host) * sizeof(cl_float), ax_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (ax_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, ay_buffer, CL_TRUE, 0, (num_bodies_host) * sizeof(cl_float), ay_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (ay_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, az_buffer, CL_TRUE, 0, (num_bodies_host) * sizeof(cl_float), az_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (az_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, mass_buffer, CL_TRUE, 0, (num_nodes_host+1) * sizeof(cl_float), mass_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (mass_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, rad_buffer, CL_TRUE, 0,  (num_bodies_host) * sizeof(cl_float), rad_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (rad_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, softening2_buffer, CL_TRUE, 0, sizeof(cl_float), &softening2_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (softening2_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, inv_opening_angle2_buffer, CL_TRUE, 0, sizeof(cl_float), &inv_opening_angle2_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (inv_opening_angle2_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, minimum_collision_velocity_buffer, CL_TRUE, 0, sizeof(cl_float), &minimum_collision_velocity_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (minimum_collision_velocity_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, num_nodes_buffer, CL_TRUE, 0, sizeof(cl_int), &num_nodes_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (num_nodes_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, num_bodies_buffer, CL_TRUE, 0, sizeof(cl_int), &num_bodies_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (num_bodies_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, boxsize_buffer, CL_TRUE, 0, sizeof(cl_float), &boxsize_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (boxsize_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, rootx_buffer, CL_TRUE, 0, sizeof(cl_float), &rootx_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (rootx_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, rooty_buffer, CL_TRUE, 0, sizeof(cl_float), &rooty_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (rooty_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, rootz_buffer, CL_TRUE, 0, sizeof(cl_float), &rootz_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR rootzx_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }  
  error = clEnqueueWriteBuffer(queue, OMEGA_buffer, CL_TRUE, 0, sizeof(cl_float), &OMEGA_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (OMEGA_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, OMEGAZ_buffer, CL_TRUE, 0, sizeof(cl_float), &OMEGAZ_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (OMEGAZ_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, sindt_buffer, CL_TRUE, 0, sizeof(cl_float), &sindt_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (sindt_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, tandt_buffer, CL_TRUE, 0, sizeof(cl_float), &tandt_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (tandt_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, sindtz_buffer, CL_TRUE, 0, sizeof(cl_float), &sindtz_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (sindtz_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, tandtz_buffer, CL_TRUE, 0, sizeof(cl_float), &tandtz_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (tandtz_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, t_buffer, CL_TRUE, 0, sizeof(cl_float), &t_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (t_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, collisions_max2_r_buffer, CL_TRUE, 0, sizeof(cl_float), &collisions_max2_r_buffer, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (collisions_max2_r_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, dt_buffer, CL_TRUE, 0, sizeof(cl_float), &dt_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (dt_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, G_buffer, CL_TRUE, 0, sizeof(cl_float), &G_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (G_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, start_buffer, CL_TRUE, 0, (num_nodes_host+1) * sizeof(cl_int), start_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (start_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, sort_buffer, CL_TRUE, 0, (num_nodes_host+1) * sizeof(cl_int), sort_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (sort_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }   
  error = clEnqueueWriteBuffer(queue, count_buffer, CL_TRUE, 0, (num_nodes_host+1) * sizeof(cl_int), count_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (count_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, children_buffer, CL_TRUE, 0,  8 * (num_nodes_host+1) * sizeof(cl_int), start_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (children_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, collisions_buffer, CL_TRUE, 0, sizeof(cl_int) * num_bodies_host * (nghostx + 1) * (nghosty + 1) * (nghostz + 1), collisions_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (collisions_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, bottom_node_buffer, CL_TRUE, 0, sizeof(cl_int), &bottom_node_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (bottom_node_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
  error = clEnqueueWriteBuffer(queue, maxdepth_buffer, CL_TRUE, 0, sizeof(cl_int), &maxdepth_host, 0, NULL, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueWriteBuffer ERROR (maxdepth_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  } 
}
...
void cl_init_set_kernel_arg_collisions_search_kernel(){
  ...
  error = clSetKernelArg(collisions_search_kernel, 6, sizeof(cl_mem), &rad_buffer);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/mass_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 7, sizeof(cl_mem), &sort_buffer);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/sort_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 8, sizeof(cl_mem), &collisions_buffer);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/collisions_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 9, sizeof(cl_mem), &children_buffer);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/children_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 10, sizeof(cl_mem), &maxdepth_buffer);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/maxdepth_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 11, sizeof(cl_mem), &boxsize_buffer);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/boxsize_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 12, sizeof(cl_mem), &num_nodes_buffer);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/num_nodes_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 13, sizeof(cl_mem), &num_bodies_buffer);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/num_bodies_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 14, sizeof(cl_int)*wave_fronts_in_collisions_search_kernel, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/children_local): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 15, sizeof(cl_int)*wave_fronts_in_collisions_search_kernel*MAX_DEPTH, NULL);
  if (error != CL_SUCCESS) {
  ...
  error = clSetKernelArg(collisions_search_kernel, 17, sizeof(cl_float)*wave_fronts_in_collisions_search_kernel*MAX_DEPTH, NULL);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/dr_cutoff_local): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 18, sizeof(cl_float)*wave_fronts_in_collisions_search_kernel, NULL);
  if (error != CL_SUCCESS) {
  ...
  error = clSetKernelArg(collisions_search_kernel, 25, sizeof(cl_int)*local_size_collisions_search_kernel, NULL);			
  if (error != CL_SUCCESS) {
    fprintf(stderr, "clSetKernelArg Error (collisions_search_kernel/wavefront_vote_local): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 26, sizeof(cl_mem), &collisions_max2_r_buffer);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/collisions_max2_r_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 27, sizeof(cl_mem), &OMEGA_buffer);
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (collisions_search_kernel/OMEGA_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
  error = clSetKernelArg(collisions_search_kernel, 28, sizeof(cl_mem), &t_buffer);
  if (error != CL_SUCCESS) {
