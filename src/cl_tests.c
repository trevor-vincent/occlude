...
int cl_tests_intersect(int p, float * x, float * y, float *z, float *r, int size){
  int intersect = 0;
  for (int i = 0; i < size; i++){
    ...
    float z2 = z[p];
    float r2 = r[p];
    if (cl_tests_particles_intersect(x1,y1,z1,r1,x2,y2,z2,r2) == 1){
      intersect = 1;
      break;
    }
  }
...
void cl_tests_print_particle_positions_with_ghosts(
				  int step,
				  cl_char * message,
				  int transfer_from_device,
				  int ghostx_col,
  ...
  float shiftx, shifty, shiftz, temp;

  printf( "\n PARTICLE INFO FOR: %s \n", message);
  printf( "ITERATION: %d \n", step);
  for (int i = 0; i < num_nodes_host + 1; i++){
    for (int gbx = -ghostx_col; gbx <= ghostx_col; gbx++)
       for (int gby = -ghosty_col; gbx <= ghosty_col; gbx++)
	 for (int gbz = -ghostz_col; gbx <= ghostz_col; gbx++)   
	   if (i < num_bodies_host){
	     cl_tests_boundaries_get_ghostbox(gbx,gby,gbz,&shiftx,&shifty,&shiftz, &temp, &temp, &temp);     
	     printf(" %d %d %d %d %.15f %.15f %.15f\n", i, gbx, gby, gbz, x_host[i] + shiftx, y_host[i] + shifty, z_host[i] + shiftz);
	   }
  }  
}
...
void cl_tests_read_simulation_time_from_device(){
  error = clEnqueueReadBuffer(queue, t_buffer, CL_TRUE, 0, sizeof(cl_float) , &t_host, 0, NULL, NULL);
  if(error != CL_SUCCESS) {
    fprintf(stderr,"clEnqueueReadBuffer ERROR (t_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
}
