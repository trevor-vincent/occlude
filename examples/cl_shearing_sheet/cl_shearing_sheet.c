...
void cl_problem_init(int num_bodies, int num_threads_tree_kernel, int num_threads_tree_gravity_kernel, int num_threads_tree_sort_kernel, int num_threads_force_gravity_kernel, int num_threads_collisions_search_kernel, int num_threads_collisions_resolve_kernel, int num_threads_boundaries_kernel, int num_threads_integrator_kernel)
{
  ...
  cl_int p = 0;
  cl_float total_mass = surface_density*boxsize_host*boxsize_host;
  cl_float mass = 0;
  ...
  while(mass<total_mass || p % WAVEFRONT_SIZE != 0 || p % local_size_integrator_kernel != 0){
  ...
  for (int i = 0; i < 8 * (num_nodes_host+1); i++)
    children_host[i] = -5;
...
/* #define TEST_KERNELS */
  ...
  /* cl_tests_print_tree_info(-1, (cl_char*) "\n TREE INFO AFTER SIMULATION \n", 1); */

  /* cl_init_print_thread_info(); */
  cl_tests_read_simulation_time_from_device();
  printf(" Number of Particles = %d\n", num_bodies_host);
  printf(" Execution time (s) = %f\n\n ", time_spent);
  printf("\n\n Simulation time = %.6f \n", t_host);
  printf(" dt = %.6f \n", dt_host);

  /* printf(" %.6f %.6f %d %d %.6f \n", surface_density, particle_density, num_bodies_host, num_bodies_host*9, time_spent); */

  cl_tests_print_particle_positions_with_ghosts(steps, (cl_char*) "\n PARTICLE POSITIONS INCL. GHOSTS AFTER SIMULATION \n", 1, 1, 1, 0);
